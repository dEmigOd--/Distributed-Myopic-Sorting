function [ new_state, new_memory, collided, stop ] = MultipleSnakeTailVer2( state, memory, params )
	...
	zero_indixes = indexes(where_zeros);
	future_occupied_zeros = false(size(zero_indixes));
	
	for direction = go_north:go_west
		actual_indexes_moving = indexes(current_directions == direction);
		future_occupied_zeros = future_occupied_zeros | ismember(zero_indixes, actual_indexes_moving + shift_in(direction + 1));
		new_memory(actual_indexes_moving + shift_in(direction + 1)) = new_memory(actual_indexes_moving);
		new_memory(actual_indexes_moving) = 0;
	end
	
	new_memory(zero_indixes(~future_occupied_zeros)) = 0;
...
function [result_direction_map] = GetDirectionMap()
	...
	if(isempty(direction_map))
		...
		direction_map(:, 1:4, 3) = [ ...
			...
			do_nothing, go_east, go_south, go_east];
		direction_map(:, 1:4, 4) = [ ...
			do_nothing, do_nothing, do_nothing, do_nothing; ...
			error, do_nothing, error, do_nothing; ...
			do_nothing,   go_south, do_nothing,   go_south; ...
			error, error, error, error];
		direction_map(:, 1:8, 5) = [ ...
			do_nothing, go_north, do_nothing, go_west, go_north, go_north, go_west, go_north; ...
			error, error, error, error, error, error, error, error; ...
		...
		direction_map(:, 1:8, 6) = [ ...
			...
			do_nothing, go_north, do_nothing, do_nothing, go_north, do_nothing, do_nothing, do_nothing; ...
		...
		direction_map(:, 1:8, 8) = [ ...
			do_nothing, go_north, do_nothing, do_nothing, go_north, go_north, do_nothing, go_north; ...
			do_nothing, error, do_nothing, do_nothing, do_nothing, error, do_nothing, do_nothing; ...
			do_nothing, do_nothing, go_south, do_nothing, go_south, do_nothing, go_south, go_south; ...
			error, error, error, error, error, error, error, error];
		direction_map(:, 1:4, 9) = [ ...
			do_nothing, go_north, do_nothing, go_north; ...
			do_nothing, go_north, do_nothing, do_nothing; ...
			do_nothing, do_nothing, go_south, go_south; ...
			error, error, error, error];
	end

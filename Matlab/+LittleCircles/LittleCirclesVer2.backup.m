function [ new_state, new_memory, collided, stop ] = LittleCirclesVer2( state, memory, params )
	...
	% we can fast and furious ignore more than one empty cell currently
		...
		[my_priority, neighbors_priority, suspected_neighbors_priority] = GetPriorities(state, neighborhood, params);		
	...
	if(~initialCovering && Any(move_intentions))
		...
		if(~initialCovering && Any(has_intentions))
			new_memory(has_intentions) = 2 + (memory(has_intentions) == 2);
		else

function [ new_state, new_memory, collided, stop ] = LittleCirclesVer3( state, memory, params )
	%LITTLECIRCLESVER3 Always try to execute circles, otherwise do covering
	...
	% input direction
	north = params.north + 1; west = params.west + 1;
	...
	if(size(state, 2) <= 2)
		% think of first column full of exiting vehicles
	...
	if (MeAndMyNeighborsSeeThingsDifferently(state, my_priority, neighbors_priority, suspected_neighbors_priority))
		printf('Potentially they can move\n');
		stop = true;
		return;
	end
	
	if(Sum(move_intentions) > 1)
		collided = true;
	...
	if(Any(move_intentions))
		...
		new_memory(state == EMPTY & ~(u_row | d_row | m1_column)) = 2;
		new_memory(state == EMPTY & (u_row | d_row)) = 1;
		% new_memory(state == EMPTY & ~(u_row | d_row) & m1_column) = 3;
		
		for direction = north:west
			move_intent = (neighborhood(:, :, direction) == EMPTY) & move_intentions;
			new_memory(find(move_intent & (state == VEXIT)) + offset(direction)) = 3;
			% new_memory(find(move_intent & (state == VCONTINUE)) + offset(direction)) = 2;
		end		
	...
	if(Any(has_intentions & ~move_intentions))
		new_memory(has_intentions & ~move_intentions) = 3;% 2 + (memory(has_intentions & ~move_intentions) ~= 3);
	end
	
	% detect cells without neighboring empty cells and re-ignite them
	no_empty_cell_neighbor = ones(size(new_memory));
...
function [problematic_view] = MeAndMyNeighborsSeeThingsDifferently(state, my_priority, neighbors_priority, suspected_neighbors_priority)
	EMPTY = 0;
	sz = size(state);
	...
	problematic_view = Any(self_priorities < max(deduced_by_others, [], 1));
end

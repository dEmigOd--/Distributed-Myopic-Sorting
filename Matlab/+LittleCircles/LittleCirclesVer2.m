function [ new_state, new_memory, collided, stop ] = LittleCirclesVer2( state, memory, params )
	...
	% actions are ranked, common knowledge is used, to detect who's supposed to move
	...
	[my_priority, neighbors_priority, suspected_neighbors_priority, no_priority] = ...
		GetPriorities(state, neighborhood, params);
	
	% remove any reasoning if in initializing state
	before_initialization = (memory < 2);
	my_priority(before_initialization) = no_priority;
	neighbors_priority(before_initialization) = no_priority;
	suspected_neighbors_priority(before_initialization) = no_priority;
	...
	[sim_state, sim_memory, ~, sim_stopped, sim_move_decisions] = ...
		CoverageAlgorithm(state, memory, params, ~before_initialization & (my_priority ~= no_priority));

	if(Sum(move_intentions | (sim_move_decisions > 0)) > 1)
		collided = true;
		fprintf('Sorting: collision detected\n');
		return;
	end
	
	temp_memory = zeros(size(memory));
	if(Any(has_intentions))
		temp_memory(has_intentions) = 2 + (memory(has_intentions) == 2);
	end
	
	% index offsets
	offset = [-1; params.n; 1; -params.n];
	
	if(Any(move_intentions))
		% we are near some one and moving it
		% set memory for moving into first or last row to 1
		temp_memory(neighborhood(:, :, 2) == EMPTY & ~(u_row | d_row) & ~move_intentions & ~before_initialization) = 2;
		temp_memory(neighborhood(:, :, 2) == EMPTY & (u_row | d_row) & ~move_intentions & ~before_initialization) = 1;
		temp_memory(state == EMPTY & ~(u_row | d_row) & ~before_initialization) = 2;
		temp_memory(state == EMPTY & (u_row | d_row) & ~before_initialization) = 1;
		
		for direction = north:west
			move_intent = (neighborhood(:, :, direction) == EMPTY) & move_intentions;
			temp_memory(find(move_intent & state == VEXIT) + offset(direction)) = 3;
			temp_memory(find(move_intent & state == VCONTINUE) + offset(direction)) = 2;
		end		
		
		temp_memory(move_intentions) = 0;		
		new_state(state == EMPTY) = state(move_intentions);
		new_state(move_intentions) = EMPTY;
	end
		
	new_memory = sim_memory;		
	% those engaging in coverage should update the state
	if(Any(sim_move_decisions > 0))
		new_state = sim_state;
		stop = sim_stopped;
	end
	new_memory(temp_memory > 0) = temp_memory(temp_memory > 0);
...
% 				(state ~= VEXIT) & (neighborhood(:, :, 6) == VEXIT)) = 3;
	...
	if(~Any(new_state(:, end) ~= VEXIT) && (new_memory(2, end) == 3))
		stop = true;
	end
end
...
function [my_priority, neighbors_priority, suspected_neighbors_priority, no_priority] = ...
		GetPriorities(state, neighborhood, params)
	VCONTINUE = params.vehicle_continue;
	VEXIT = params.vehicle_exit;
	...
	% field indexes
	MY_STATUS = 1;
	DIRECTION = MY_STATUS + 1;
	NEIGHBORS_EXITING = DIRECTION + 1;
	NEIGHBORS_CONTINUE = NEIGHBORS_EXITING + 1;
	REQUIREMENTS_COLUMN = NEIGHBORS_CONTINUE + 1;
	REQUIREMENTS_ROW = REQUIREMENTS_COLUMN + 1;
	FIELD_COUNT = REQUIREMENTS_ROW;
	...
	no_priority = PRIORITIES_COUNT + 1;
	
	priorities = cell(PRIORITIES_COUNT, FIELD_COUNT);
	...
	priorities(       down_right_3, :) = {        [],  WEST,         {1},     {}, {-M0_COLUMN},        {}};
	priorities(       down_right_4, :) = { VCONTINUE,  WEST,         {8},    {1},           {},        {}};
	priorities(       down_right_5, :) = { VCONTINUE, SOUTH,         {4},     {},           {},        {}};
		...
		for npi = PRIORITIES_COUNT:-1:1
			if direction ~= priorities{npi, DIRECTION}
				...
				if(~isempty(priority_entry))
					current_priority = ConvertPriority(state, neighborhood, neighborhood_indexes, ni_lookup, priority_entry, params);
...
function [current_priority] = ConvertPriority(state, neighborhood, neighborhood_indexes, ni_lookup, priorities_entry, params)
	MY_STATUS = 1;
	DIRECTION = 2;
	NEIGHBORS_EXITING = 3;
	NEIGHBORS_CONTINUE = 4;
	REQUIREMENTS_COLUMN = 5;
	REQUIREMENTS_ROW = 6;

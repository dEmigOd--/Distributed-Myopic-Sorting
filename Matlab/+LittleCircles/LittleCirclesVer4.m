function [ new_state, new_memory, collided, stop ] = LittleCirclesVer4( state, memory, params )
	...
	memory(m1_column & d2_row & (neighborhood(:, :, 3) == EMPTY) & ...
		(state ~= VEXIT) & (neighborhood(:, :, 16) == VEXIT)) = 3;
	% patch 1 in the right bottom corner
	memory(m_column & (memory == 0) & (state == VEXIT)) = 3;
...
function [new_column_rule, is_impossible, do_not_care] = ...
	...
	ME = 25; % my index in whole neighborhood

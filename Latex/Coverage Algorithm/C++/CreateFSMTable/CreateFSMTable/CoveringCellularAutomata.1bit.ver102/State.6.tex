...
\begin{document}
	\begin{tikzpicture}
		...
		% set cell values
		\node (node_00) at (0 + \valuexoffset, 1 + \valueyoffset) {0, N};
		\node (node_10) at (1 + \valuexoffset, 1 + \valueyoffset) {0, $\emptyset$};
		\node (node_20) at (2 + \valuexoffset, 1 + \valueyoffset) {0, $\emptyset$};
		\node (node_01) at (0 + \valuexoffset, 0 + \valueyoffset) {1, N};
		\node (node_11) at (1 + \valuexoffset, 0 + \valueyoffset) {1, $\emptyset$};
		\node (node_21) at (2 + \valuexoffset, 0 + \valueyoffset) {1, $\emptyset$};
		
		% set final state
		\pgfmathsetmacro{\slightoffset}{0.05}
		\draw[rounded corners] (0 + \slightoffset, 0 + \slightoffset) rectangle (1 - \slightoffset, 1 - \slightoffset);
		...
		% hatch error cells
		\foreach \x/\y in {1/1, 2/1, 1/0, 2/0}
		{

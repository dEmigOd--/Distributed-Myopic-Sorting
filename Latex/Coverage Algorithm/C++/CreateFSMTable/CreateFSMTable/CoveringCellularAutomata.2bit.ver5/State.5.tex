...
\begin{document}
	\begin{tikzpicture}
		...
		\node (node_30) at (3 + \valuexoffset, 3 + \valueyoffset) {0, W};
		\node (node_01) at (0 + \valuexoffset, 2 + \valueyoffset) {2, $\emptyset$};
		\node (node_11) at (1 + \valuexoffset, 2 + \valueyoffset) {2, $\emptyset$};
		\node (node_31) at (3 + \valuexoffset, 2 + \valueyoffset) {2, $\emptyset$};
		\node (node_02) at (0 + \valuexoffset, 1 + \valueyoffset) {3, N};
		\node (node_12) at (1 + \valuexoffset, 1 + \valueyoffset) {2, E};
		\node (node_32) at (3 + \valuexoffset, 1 + \valueyoffset) {3, $\emptyset$};
		\node (node_03) at (0 + \valuexoffset, 0 + \valueyoffset) {3, $\emptyset$};
		\node (node_13) at (1 + \valuexoffset, 0 + \valueyoffset) {1, $\emptyset$};
		\node (node_33) at (3 + \valuexoffset, 0 + \valueyoffset) {3, $\emptyset$};
		
		% hatch error cells
		\foreach \x/\y in {2/3, 2/2, 2/1, 2/0}
		{

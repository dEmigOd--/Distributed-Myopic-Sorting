...
\begin{document}
	\begin{tikzpicture}
		...
		\node (node_21) at (2 + \valuexoffset, 2 + \valueyoffset) {3, S};
		...
		\draw[rounded corners] (2 + \slightoffset, 2 + \slightoffset) rectangle (3 - \slightoffset, 3 - \slightoffset);
		\draw[rounded corners] (2 + \slightoffset, 1 + \slightoffset) rectangle (3 - \slightoffset, 2 - \slightoffset);
		
		% hatch error cells
		\foreach \x/\y in {0/3, 3/3, 0/2, 3/2, 0/1, 3/1, 0/0, 1/0, 3/0}
		{

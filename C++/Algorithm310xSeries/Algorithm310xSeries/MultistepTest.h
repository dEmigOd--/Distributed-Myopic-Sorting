...
namespace Phd
{
	class MultistepTest : public BasicTest
	{
	...
	protected:
		typedef AbstractParameter& (MultistepTest::* ParamFnc)() const;

		MultistepTest(SimulationParameters params, Parameters testParams, std::shared_ptr<Phd::Algorithm> algo, std::shared_ptr<std::mt19937> gen, 
			...
			for (; !refParam.end(); ++refParam)
			{
				...
				for (unsigned run = 0; run < numSimulations; ++run)
				{
					...
					std::shared_ptr<Phd::LLGrid> grid = 
						gridCreator->CreateGrid(CreateRandomRoad(*n(), *m(), std::min(*k(), *n() * (*m()) - *n() + 1), *n() - 1), mask, algo);

...
namespace Phd
{
	class VariableExitingNumberTest : public BasicTest
	{
	...
	public:
		VariableExitingNumberTest(SimulationParameters params, Parameters testParams, std::shared_ptr<Phd::Algorithm> algo, std::shared_ptr<std::mt19937> gen,
			...
			unsigned maxIterations = std::numeric_limits<unsigned>::max()) override
		{
			...
			for (; !ones().end(); ++ones())
			{
				...
				for (unsigned run = 0; run < numSimulations; ++run)
				{
					...
					auto grid = gridCreator->CreateGrid(CreateRandomRoad(*n(), *m(), std::min(*k(), *n() * (*m()) - *ones()), *ones()), mask, algo);
